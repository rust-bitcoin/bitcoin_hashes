...
impl Hash {
    /// Creates an engine with the given `midstate` and `length` of already
    /// hashed input data. This function should not be used directly unless you
    /// really know what you're doing.
    pub fn engine_with_state(midstate: [u8; 32], length: usize) -> HashEngine {
        let mut h = [0; 8];
...
mod tests {
    ...
    fn engine_with_state() {
        let mut engine = sha256::Hash::engine();
        let midstate_engine = sha256::Hash::engine_with_state(engine.midstate(), 0);
        ...
        for data in data_vec {
            ...
            let mut midstate_engine = sha256::Hash::engine_with_state(engine.midstate(), engine.length);
            assert_eq!(engine.h, midstate_engine.h);
        ...
        let midstate_engine = sha256::Hash::engine_with_state(MIDSTATE, 64);
